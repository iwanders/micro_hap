...
type Control = crate::HapControlChannel<Mutex, CONTROL_CHANNEL_N>;
...
async fn reset_context(ctx: &mut HapPeripheralContext<'static>) {
    ctx.handle_disconnect().await;
    {
        let info = &mut ctx.pair_ctx.borrow_mut().info;
        // We need real commissioning for this.
        // from recording 2025_08_24_1639.
        info.salt = [
...
> {
    ...
    impl crate::AccessoryInterface for LightBulbAccessory {
        async fn read_characteristic<'a>(
            ...
            if char_id == crate::ble::services::lightbulb::CHAR_ID_LIGHTBULB_NAME {
                self.name.read_characteristic_into(char_id, output)
            } else if char_id == crate::ble::services::lightbulb::CHAR_ID_LIGHTBULB_ON {
        ...
        async fn write_characteristic(
            ...
            if char_id == crate::ble::services::lightbulb::CHAR_ID_LIGHTBULB_ON {
                let value = data
                    .get(0)
    ...
    // Write to name... three times... oh, different handles.
    {
        ...
        if scenario == TestScenario::FailResume {
            ...
            support.add_random(&[0x4b, 0xff, 0x03, 0x1d, 0x7d, 0x97, 0x5f, 0x01]);
            support.add_random(&[0x4b, 0xff, 0x03, 0x1d, 0x7d, 0x97, 0x5f, 0x01]);
            let mut s = ctx.pair_ctx.borrow_mut();
            s.server.ble_session_cache = Default::default();
        }
        {
            let incoming_data: &[u8] = &[
            ...
            let outgoing: &[u8] = if scenario == TestScenario::FailResume {
                &[
                    ,
                ]
            } else {
                &[
    ...
    if scenario == TestScenario::RemoveAccessory {
        ...
        // This CharacteristicExecuteWrite payload is from 2025_08_22_1430_homekitadk_pair_disconnect_connect_toggle.txt
        {
            ...
            let mut execute_write_encr = vec![
                0;
                execute_write_plain.len()
                    + crate::crypto::aead::CHACHA20_POLY1305_KEY_BYTES
            ];
            ...
            let outgoing_plain: &[u8] =
                &[0x02, 0xcc, 0x00, 0x05, 0x00, 0x01, 0x03, 0x06, 0x01, 0x02];
    ...
    if TEST_PAIR_RESUME_AFTER_DISCONNECT {
...

        {
            let incoming_data: &[u8] = &[
                ...
                0x06, 0x0e, 0x08, 0x75, 0x78, 0x74, 0x32, 0xa5, 0x27, 0x50, 0xad, 0x05, 0x10, 0x1c,
                0xbc, 0xd2, 0x51, 0x86, 0x89, 0x82, 0x1b, 0x72, 0x5f, 0x24, 0x44, 0xd4, 0x83, 0x07,
                0xb2, 0x03, 0x20, 0xb3, 0x26, 0x3c, 0x1d, 0x18, 0xaa, 0x5e, 0xea, 0x43, 0xb4, 0x74,
                0xcc, 0x73, 0x37, 0x8c, 0xd2, 0x15, 0x23, 0x2f, 0x35, 0xc2, 0xb7, 0xda, 0x14, 0x0d,
                0xa4, 0x8f, 0xef, 0xaf, 0x6f, 0x04, 0x32, 0x09, 0x01, 0x01,
            ];
            let outgoing: &[u8] = &[
                0x02, 0x5f, 0x00, 0x8e, 0x00, 0x01, 0x8c, 0x06, 0x01, 0x02, 0x03, 0x20, 0x8f, 0x40,
                ...
                0x28, 0x5f, 0x05, 0x65, 0xc4, 0x51, 0xaf, 0x47, 0x0f, 0x8b, 0xa6, 0x4d, 0x0f, 0xfc,
                0x66, 0x16, 0x61, 0xb3, 0x34, 0xa7, 0xde, 0xa1, 0x5f, 0x76, 0x80, 0x5f, 0xcb, 0x4e,
                0xf7, 0xee, 0x3e, 0x45, 0x4c, 0x44, 0xe7, 0xa3, 0xfc, 0x3f, 0x4e, 0x09, 0xbe, 0xe4,
                0x8b, 0xf0, 0xdc, 0x26, 0xaa, 0x56, 0x5b, 0x57, 0xbb, 0x39, 0x91, 0x28, 0xe2, 0xe0,
                0x33, 0xf7, 0xf0, 0xe5, 0x1b, 0x02, 0x88, 0xeb, 0x17, 0x39, 0xea, 0xe4, 0x63, 0x67,
                0x87, 0xf6, 0xa8, 0xac, 0x69, 0x13, 0x32, 0xbd, 0xd4, 0xdf, 0xd0, 0xd7, 0xb1, 0x6f,
                0x47, 0xf2, 0x93, 0xfc, 0xeb, 0x41, 0xd8, 0x15, 0x00, 0x76, 0xd2, 0xc5, 0x84, 0x01,
                0x58, 0x4b, 0x8c, 0x45, 0xd7, 0x59, 0x7d,
            ];
            eprintln!("\n\n\nPair verify \n\n\n");
            ctx.handle_write_incoming_test(
                &mut support,
    ...
    // now? At least, I think we're good now!
    {
        ...
        {
            let incoming_data: &[u8] = &[
                0, 2, 67, 35, 0, 73, 0, 1, 68, 6, 1, 1, 0, 1, 6, 14, 8, 144, 211, 141, 209, 124,
                96, 75, 168, 5, 16, 204, 246, 162, 216, 238, 170, 246, 246, 202, 252, 20, 130, 14,
                68, 224, 53, 3, 32, 156, 46, 82, 207, 178, 196, 34, 177, 187, 135, 208, 31, 204,
                34, 131, 249, 20, 65, 87, 22, 67, 11, 248, 130, 31, 116, 93, 36, 133, 28, 126, 30,
                9, 1, 1,
            ];
            let outgoing: &[u8] = &[
                ,
            ];
            eprintln!("\n\n\nPair verifyzzzz \n\n\n");
            ctx.handle_write_incoming_test(
                &mut support,
    ...
    if scenario == TestScenario::DisconnectedValueModification {
        ...
        control_channel
            .get_sender()
            .characteristic_changed(crate::ble::services::lightbulb::CHAR_ID_LIGHTBULB_ON)
            .await;
        ...
        //  advertisement id and iid are correct.
        //  value is correct.
        //  broadcast key is correct
        // Issue is likely the gsn, since that is used as the nonce?
        //
        //
