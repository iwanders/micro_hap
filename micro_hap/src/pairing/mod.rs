...
pub struct SetupInfo {
    ...
    #[cfg_attr(feature = "serde", serde(with = "serde_support::arrays"))]
    pub verifier: [u8; 384],
}
...
pub mod test {
    ...
    fn test_setupinfo_serde() {
        ...
        let info = SetupInfo { salt, verifier };
        // Test serialization roundtrip of the setup info.
        let j = serde_json::to_string(&info).unwrap();
        let back: SetupInfo = serde_json::from_str(&j).unwrap();
        assert_eq!(info, back);
        assert_eq!(back.salt, salt);
        assert_eq!(back.verifier, verifier);
    }
}
